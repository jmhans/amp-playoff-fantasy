...
async function checkCalebWilliams() {
  ...
  console.log('Checking Caleb Williams stats...\n');
  
  // Find Caleb Williams
  const [player] = await db.select().from(players).where(eq(players.name, 'Caleb Williams')).limit(1);
  
  if (!player) {
    console.log('Caleb Williams not found');
    return;
  }
  ...
  // Passing: 1 pt per 25 yards (no fractional), 4 pts per TD, 1 pt per 2pt
  ...
  // Rushing: 1 pt per 10 yards (no fractional), 6 pts per TD, 2 pts per 2pt
  ...
  // Receiving: 1 pt per 10 yards (no fractional), 6 pts per TD, 2 pts per 2pt
